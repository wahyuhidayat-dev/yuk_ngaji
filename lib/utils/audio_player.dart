...
class AudioPlayerState extends State<AudioPlayer> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(50), color: Colors.white54),
      child: _buildControl(),
    );
  }
  ...
  Widget _buildControl() {
    ...
    if (_audioPlayer.state == ap.PlayerState.playing) {
      icon = const Icon(Icons.pause, color: colorDarkBlue, size: 24);
      color = whiteColor.withOpacity(0.1);
    } else if (_audioPlayer.state == ap.PlayerState.stopped) {
      // final theme = Theme.of(context);
      icon = const Icon(Icons.play_arrow, color: colorDarkBlue, size: 24);
      color = whiteColor.withOpacity(0.1);
